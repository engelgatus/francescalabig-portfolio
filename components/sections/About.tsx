...
import FallingAlice from '@/components/assets/FallingAlice';
import { STATS } from '@/lib/constants';

export default function About() {
  return (
    <section id="about" className="relative py-32 bg-canvas z-20 overflow-visible">
      <div 
      ...
      <Container>
        <div className="grid grid-cols-1 md:grid-cols-12 gap-12 items-center relative">
          {/* Left Column: Existing Content (Cols 2-7) */}
          <div className="col-span-1 md:col-start-2 md:col-span-6 relative z-30">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {STATS.map((stat, index) => (
                <Card key={index} className="text-center hover:shadow-dream transition-shadow">
                  <div className="text-3xl md:text-3xl font-bold text-heart mb-2">
                    {stat.value}
                  </div>
          ...
          {/* Right Column: Nini Placeholder (Cols 8-11) */}
          <div className="col-span-1 md:col-start-8 md:col-span-4 relative z-10!">
            <FallingAlice className="-top-20 right-0 w-full h-full -z-10!" />
            
            <div className="relative z-20 aspect-3/4 w-full bg-ink/5 border-2 border-dashed border-ink/10 rounded-2xl flex items-center justify-center overflow-hidden group">
              <div className="text-ink/30 font-medium text-lg group-hover:rotate-0 transition-transform">
                Picture of Nini
              </div>
            </div>
          </div>
